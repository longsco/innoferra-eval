...
log "== launch preview2 + DSpark (port) on GPUs 4-7, TRAINING_COMPAT=${TC:-1} =="
$DOCKER rm -f m31-b2 >/dev/null 2>&1; sleep 4
MODEL_PATH=$M NAME=m31-b2 PORT=19291 GPUS=4,5,6,7 TP_SIZE=4 EP_SIZE=4 DP_SIZE=4 CHUNK=65536 MAXREQ=128 SPEC=dspark DEV_SRC=$SRC TRAINING_COMPAT=${TC:-1} FOLLOW=1 WAIT=2400 bash "$K/launch.sh" | grep -E "FATAL|Error|Exception|Traceback|healthy after|max_total_num_tokens|Engine startup|DSpark|dspark|draft|Initialized|resolved argv|WARN" | grep -vE "server_args=" | cut -c1-300 | tail -40
if curl -sf -m 5 http://127.0.0.1:19291/health >/dev/null; then
  ...
  log "== quick TPM c1,c8 (DSpark) =="; TAG=preview2-dspark-port-tc${TC:-1}-p19291 PORT=19291 bash "$K/bench_tpm.sh" "1 8" | grep -E "^80k-warm"
