...
TS=$(date -u +%Y%m%dT%H%M%SZ); mkdir -p "$OUT"; CSV="$OUT/tpm-$TS.csv"; LOG="$OUT/tpm-$TS.log"
BSV(){ $DOCKER run --rm --network host -v "$MODEL_PATH":/models:ro "$IMAGE" python3 -m sglang.bench_serving \
...
echo "== bench $TS  frame: gsp ${SYS_LEN}/${Q_LEN}/${OUT_LEN} · warm ${WARM_N}@c8 · grid: $GRID ==" | tee "$LOG"
...
for C in $GRID; do
  ...
  echo "80k-warm,$C,$npc,$row" | tee -a "$CSV"; echo "   ($(( $(date +%s)-t0 ))s)" | tee -a "$LOG"
done
