...
# 9. scheduler_components/kv_events_publisher.py: the KV-metrics publish to the Dynamo worker's collector was a BLOCKING zmq send.
#    On the second Dynamo worker of the node the collector stops draining after the first long prefill, and the blocking send stalled
#    all 4 DP schedulers until the 300 s watchdog restarted the container (3 times on 2026-09-26; py-spy dump at 15:15Z). Non-blocking + drop.
patch("managers/scheduler_components/kv_events_publisher.py", [('        if not self.send_metrics_from_scheduler.closed:\n            sock_send(self.send_metrics_from_scheduler, kv_metrics)\n', '        if not self.send_metrics_from_scheduler.closed:\n            # innoferra: NEVER block the scheduler on the metrics socket. On the second Dynamo worker of a node the collector stops\n            # draining after the first long prefill; this blocking send then stalled every DP scheduler until the 300 s watchdog\n            # (py-spy 2026-09-26 15:15Z: zmq_msg_send <- emit_kv_metrics <- report_prefill_stats). KV metrics are advisory: drop.\n            try:\n                sock_send(self.send_metrics_from_scheduler, kv_metrics, flags=zmq.NOBLOCK)\n            except zmq.Again:\n                self._kv_metrics_dropped = getattr(self, "_kv_metrics_dropped", 0) + 1\n                if self._kv_metrics_dropped in (1, 100, 10000):\n                    import logging as _logging\n                    _logging.getLogger(__name__).warning("kv metrics socket would block; dropped %d metric message(s) so far", self._kv_metrics_dropped)\n')])
