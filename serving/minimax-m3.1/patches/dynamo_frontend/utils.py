...
def extract_mm_urls(
    ...
    for msg in messages:
        # innoferra patch (2026-09-26): collect media from EVERY role, not only "user". The chat template renders a
        # placeholder for each image part in any message (MiniMax-M3 tool results carry screenshots), so a
        # user-only walk under-supplies data -> the worker raises "More 'IMAGE' tokens found than corresponding data"
        # (mixed user+tool images -> 500 / empty stream) and tool-only images were silently never seen by the model.
